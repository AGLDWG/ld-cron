    ...
    if len(MESSAGES) > 0:
        ...
        if len(sys.argv) > 1:
            if sys.argv[1] == '-e' or sys.argv[1] == '--email':
                ...
                send_email_via_mailjet(message, '[agldwg cron] Failures', pwds.emails, pwds.secrets)
    else:
        print('No failures')
        ...
        if len(sys.argv) > 1:
            if sys.argv[1] == '-e' or sys.argv[1] == '--email':
                ...
                send_email_via_mailjet('No errors for this report', '[agldwg cron] No failures', pwds.emails, pwds.secrets)
